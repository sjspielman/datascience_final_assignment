...
server <- function(input, output, session) {
    ...
    output$nyt_plot <- renderPlot({
        nyt_data_subset() %>%
            ...
            labs(title = paste(input$which_state, "Cases & Deaths"),
                               x = "Date",
                               y = "Cumulative Count",
                               color = "Type") -> my_nyt_plot
        
        ## Deal with input$y_scale choice
        if (input$y_scale == "Log"){
            my_nyt_plot <- my_nyt_plot + scale_y_log10()
        }
        
        ## Deal with input$facet_county
        if (input$facet_county == "Yes") my_nyt_plot <- my_nyt_plot + facet_wrap(~county, scales = "free_y")
 ...
 if (input$which_theme == "Minimal") my_nyt_plot <- my_nyt_plot + theme_minimal()
        ...
        my_nyt_plot + theme(axis.text = element_text(size = 12),
